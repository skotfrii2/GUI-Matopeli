...
void GameBoard::moveFood()
{
    ...
    std::uniform_int_distribution<int> width_dist(2, width_ - 1);
    std::uniform_int_distribution<int> height_dist(2, height_ - 1);
    width_dist(rng_);
    height_dist(rng_);
    while (true)
    {
        ...
        {
            // Bool-arvo, joka signaloi
            // updateSnake -metodille
            // ruoan sijainnin päivityksen graafisesti
            isFood = true;
            return;
        }
    }
